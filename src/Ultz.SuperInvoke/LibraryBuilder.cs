...
namespace Ultz.SuperInvoke
{
    public static class LibraryBuilder
    {
            ...
            AssemblyNameDefinition name = null)
        {
            ...
            {
                var opts = item;
                var thisName = $"Ultz.Private.SIG.{item.Type.Namespace}";
        ...
        public static TypeDefinition CreateImplementation(ref BuilderOptions opts, ModuleDefinition module)
        {
            ...
            if (type.IsClass && type.IsAbstract && type.IsSubclassOf(typeof(NativeApiContainer)) || type.IsInterface)
            {
                var def = new TypeDefinition($"Ultz.Private.SIG.{type.Namespace}",
                    ...
                    type.IsClass ? Utilities.GetReference(type) : Utilities.GetReference(typeof(NativeApiContainer)));
                var impl = CreateImplementation(ref opts, typeof(NativeApiContainer).GetMethod("Load"), module);
                var ctor = CreateConstructor(opts.Type, impl.Count, module);
                ...
                if (opts.IsPInvokeProxyEnabled)
                    def.CustomAttributes.Add(new CustomAttribute(
                        Utilities.GetReference(typeof(PInvokeProxyAttribute).GetConstructor(new Type[0]))));
                return def;
            }
        ...
        public static MethodDefinition CreateConstructor(Type type, int slots, ModuleDefinition module)
        {
            ...
            ctor.Parameters.Add(new ParameterDefinition(Utilities.GetReference(typeof(NativeLibrary))));
            ...
            ModuleDefinition mod)
        {
            ...
            for (var i = 0; i < methods.Length; i++)
            {
                ...
                var ep = NativeApiAttribute.GetEntryPoint(attr, mainAttr);
                ...
                EmitEntryPoint(il, Utilities.GetReference(load), i, ep);
                ...
                if (opts.IsPInvokeProxyEnabled)
                    ret.AddRange(PInvokeGenerator.Generate(ep, paramTypes, retType,
                        NativeApiAttribute.GetCallingConvention(attr, mainAttr), mod));
            }

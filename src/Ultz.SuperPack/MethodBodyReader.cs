...
namespace Ultz.SuperPack
{
    internal class MethodBodyReader
    {
        ...
        private unsafe CallSite ReadMethodSignature (byte[] sig)
        {
            var b = new ByteBuffer(sig);
            var convention = (CallingConventions)b.ReadByte();
            var parameters = new ParameterDefinition[b.ReadCompressedUInt32()];
            var returnType = (MetadataT)
        }
